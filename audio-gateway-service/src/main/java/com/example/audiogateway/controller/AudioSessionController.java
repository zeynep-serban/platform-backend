 ...
 *
 * <p>3-AI mutabakat (Codex {@code 019e879c}/{@code 019e8846}/{@code 019e887c} + Mavis msg
 * {@code 78}):
 ...
 * - claim adı configurable ({@code audio.gateway.jwt.tenant-claim}); backend canonical
 *   pattern (common-auth/AuthenticatedUserLookupService) {@code companyId}/{@code userId};
 *   absent → fail-closed 403 (sessiz "unknown" YASAK)
 */
@RestController
...
public class AudioSessionController {
    ...
    public Mono<ResponseEntity<?>> startSession(
        ...
        // 4. tenantId / userId JWT-derived (NEVER client-trusted)
        if (jwt == null) {
            return Mono.just(ResponseEntity
        ...
        // 5. Build response (internal headers wiring PR-queue-01)
